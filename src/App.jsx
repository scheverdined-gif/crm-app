...
function CoachView({ coach, coaches, students, sessions, session, setSession, student, setStudent, onNewSession, onNewStudent, onEditSession, onDeleteSession, onEditStudent, onDeleteStudent, onSaveAttendance, onLogout }) {
  ...
  const today = mySessions.filter(s => s.date === "2026-06-28");
  const upcoming = mySessions.filter(s => s.date > "2026-06-28");
  const past = mySessions.filter(s => s.date < "2026-06-28");
    ...
    <div className="phone-frame">
      <div className="phone-screen" style={{ "--accent": disc.color }}>
        ...
        <main className="coach-main">
          {tab === "today" && (
            <TodayTab today={today} upcoming={upcoming} past={past} onOpen={setSession} disc={disc} />
          )}
          {tab === "students" && (
...
function TodayTab({ today, upcoming, past, onOpen, disc }) {
  return (
    <div className="tab-content">
      {today.length > 0 && (
        <Section title="Сегодня" count={today.length}>
          {today.map(s => <SessionCard key={s.id} s={s} onOpen={onOpen} />)}
        </Section>
      )}
...
function AdminOverview({ students, sessions, coaches, branchFilter }) {
  ...
  const activeToday = sessions.filter(s => s.date === "2026-06-28").length;
