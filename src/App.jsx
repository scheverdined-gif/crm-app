...
// Типы тренировок: групповая, индивидуальная и спортивная группа (фиксированная длительность — 2 часа)
const SESSION_TYPES = {
  group:       { label: "Групповая",        short: "ГРУППА",     fixedDuration: null },
  individual:  { label: "Индивидуальная",   short: "ИНДИВИДУАЛ", fixedDuration: null },
  sport_group: { label: "Спортивная группа", short: "СПОРТ. ГРУППА", fixedDuration: 120 },
};
...
function SessionModal({ coaches, students, defaultCoachId, defaultBranchId, initialSession, onClose, onSave, onSaveRecurring }) {
  ...
  const pool = students.filter(s => coachesOfStudent(s).includes(coachId) && s.branchId === branchId);

  function applyType(next) {
    setType(next);
    if (SESSION_TYPES[next].fixedDuration) setDuration(SESSION_TYPES[next].fixedDuration);
  }
  ...
  function handleSave() {
    ...
    if (mode === "recurring") {
      ...
      onSaveRecurring({
        type, discipline, coachId, branchId, location: location.trim(), studentIds, note: note.trim(),
        duration: Number(duration), slots, startDate, weeksAhead: Number(weeksAhead),
      });
      return;
    }
  ...
  const durationLocked = Boolean(SESSION_TYPES[type].fixedDuration);
      ...
      <div className="modal-body">
        ...
        <div className="form-row-2">
          <FormField label="Тип">
            <div className="segmented segmented-3">
              <button className={type === "group" ? "seg active" : "seg"} onClick={() => applyType("group")}>Группа</button>
              <button className={type === "individual" ? "seg active" : "seg"} onClick={() => applyType("individual")}>Инд.</button>
              <button className={type === "sport_group" ? "seg active" : "seg"} onClick={() => applyType("sport_group")}>Спорт. группа</button>
            </div>
          </FormField>
        ...
        <div className="form-row-2">
          ...
          <FormField label="Длительность, мин">
            <input
              className="form-input" type="number" min={15} step={15}
              value={duration} disabled={durationLocked}
              onChange={e => setDuration(e.target.value)}
            />
            {durationLocked && <div className="form-hint">Фиксировано для спортивных групп — 2 часа</div>}
          </FormField>
        </div>
...
.segmented-3 .seg { font-size: 11px; padding: 8px 2px; }
